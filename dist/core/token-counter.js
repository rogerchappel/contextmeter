...
function countWordTokens(word) {
    if (word.length === 0)
        return 0;
    if (COMMON_TOKENS.has(word.toLowerCase()))
        return 1;
    if (word.length <= 4)
        return 1;
    if (word.length <= 12)
        return Math.ceil(word.length / 12);
...
export function getTokensPerChar(language) {
    return RATIOS[language.toLowerCase()] ?? 1 / 3.6;
}
export function countTokensForLanguage(text, language) {
