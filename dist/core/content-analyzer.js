...
export function detectBoilerplate(content, filePath) {
    if (!content)
        return [];
    ...
    for (const idx of boilerplateLines) {
        for (const pattern of BOILERPLATE_PATTERNS) {
            if (pattern.regex.test(lines[idx]))
...
export function detectStaleReferences(content, filePath) {
    if (!content)
        return [];
    const findings = [];
...
export function detectVerboseContent(content, filePath) {
    if (!content)
        return [];
    const findings = [];
    const lines = content.split('\n');
    ...
    for (const file of files) {
        ...
        for (const line of lines) {
            ...
            if (trimmed.length < 20)
                continue;
            if (COMMON_IMPORT_EXPORT.test(trimmed))
                continue;
...
export function analyzeFile(filePath, content, tokens, language) {
...
export function analyzeContent(files, totalTokens) {
    const allFindings = [];
